...
\subsection{Método detección rápida de objetos usando características simples utilizando el clasificador AdaBoost en forma de cascada}\label{ViolaJones}
...
Las características se guardan en un vector, llamado vector de características, donde la dimensión del vector es el número de características que describen en este caso, la mano.  

En este trabajo se utilizan características geométricas. Para extraerlas se hace uso de la envolvente convexa y de los defectos de convexidad. A continuación se definen los conceptos anteriores.

Sea $C$ un conjunto en el espacio euclidiano $\Re^d$, $C$ es un conjunto convexo \footnote{Weisstein, Eric W. "Convex." From MathWorld--A Wolfram Web Resource. http://mathworld.wolfram.com/Convex.html} si contiene todos los segmentos de línea que unen a cualquier par de puntos pertenecientes al conjunto. Donde $d$ es la dimensión del espacio euclidiano. 

Sea $C$ un conjunto de puntos en el plano Euclidiano, la envolvente convexa de $C$ es el conjunto convexo más pequeño que contiene a todos los puntos en $C$. 
